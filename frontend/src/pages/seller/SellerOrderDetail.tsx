...
const STATUS_ACTIONS: { status: string; label: string; color: string }[] = [
  { status: 'menunggu_pengirim', label: 'Mark as Ready', color: 'bg-blue-600 hover:bg-blue-500' },
  { status: 'sedang_dikirim', label: 'Mark as Shipped', color: 'bg-purple-600 hover:bg-purple-500' },
...
export default function SellerOrderDetail() {
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    ...
    setError('');
    try {
      const res = await orderApi.updateOrderStatus(order.id, newStatus);
      setOrder(res.order);
    } catch (err: unknown) {
  ...
  return (
    <div>
      <Link to="/dashboard/seller/orders" className="text-sm text-teal-600 hover:underline">
        &larr; Back to orders
      </Link>
          ...
          {order.address && (
            <div className="mt-6">
              ...
              <div className="mt-3 rounded-lg border border-slate-200 bg-white p-3 text-sm">
                <p className="font-medium text-slate-800">{order.address.recipientName} - {order.address.phone}</p>
                <p className="text-xs text-slate-500">{order.address.street}, {order.address.city}, {order.address.province} {order.address.postalCode}</p>
              </div>
            </div>
          )}
